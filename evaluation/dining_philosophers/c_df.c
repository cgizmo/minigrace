...
pthread_t start_philosopher(char *name, int fork1, int fork2) {
    pthread_t thread;
    philosopher_params *params = malloc(sizeof(philosopher_params) 
...
void philosopher_thread(void *params_) {
    ...
    while (1) {
        ...
        eat(params->name);
        return_fork(params->name, params->fork2);
        return_fork(params->name, params->fork1);
    }
}
...
void return_fork(char *name, int fork) {
